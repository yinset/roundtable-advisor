...
> **关于年报与 Token：** 为最大化节省输入侧 token，建议先将公司**年度报告（PDF 等）转换为 Markdown**（可用 OCR、文档转换或专用工具），再投喂大模型。相对直接贴原始 PDF/大段复制，**通常可节约约 50%～80% 的 token 用量**（视版面、图表与冗余多少而定），且便于模型检索段落。

```
roundtable-advisorV1.1/
